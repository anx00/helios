...
> **Update**: Minimum lowered to `$0.01` — the $1 minimum is a Polymarket UI restriction only. The CLOB API accepts much smaller orders (market-specific `min_order_size`). Both paper broker and backtest simulator now default to `$0.01`.

**Changes made:**

1. **`core/backtest/simulator.py`** (+25 lines)
   - `TakerModel.execute()`: added `min_order_usd` param; bumps size to meet minimum
   - `ExecutionSimulator.__init__()`: added `min_order_usd=0.01` default (API min, not UI $1)
   - Maker order enforcement: same minimum applied to passive orders
